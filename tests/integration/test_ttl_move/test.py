...
def wait_until_move_is_finished(node, table_name, retries=10, sleep_sec=1):
    ...
    # Move didn't finish within the given timeout - we should fail
    assert False, "Move hasn't finished in retries={} each sleep_sec={}".format(retries, sleep_sec)

def get_disk_names_of_active_parts(node, table_name, partition=None):
...
def get_details_of_active_parts(node, table_name, partition=None):
    and_with_partition = '' if partition is None else "AND partition='{}'".format(partition)
...
def test_inserts_and_moves(started_cluster, engine, ttl_rule, request):
    ...
    try:
        ...
        time.sleep(SOON_SECONDS + 1)    # wait for expiration and a bit more for MOVE to kick in
        wait_until_move_is_finished(node1, table_name)
        assert get_details_of_active_parts(node1, table_name) == {
            'external': {'rows': 40},
        }
        ...
        # Check 2. Upon expiration
        time.sleep(SOON_SECONDS + 1)    # wait for expiration
        wait_until_move_is_finished(node1, table_name)
        ...
        time.sleep(1)    # wait a bit for MOVE to kick in
        wait_until_move_is_finished(node1, table_name)
...
def test_delete(started_cluster, engine, ttl_rule, request):
    ...
    try:
        ...
        node1.query(f"SYSTEM START MOVES {table_name}")
        time.sleep(SOON_SECONDS*2 + 1)    # wait for expiration and a bit more for MOVE to kick in (it won't)
        wait_until_move_is_finished(node1, table_name)
        assert node1.query(f"SELECT count() FROM {table_name}").strip() == "60"
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            "MergeTree()",
            "TTL d1 TO DISK 'external'",
            "MODIFY TTL d1 + INTERVAL 15 MINUTE DELETE",
        ...
        pytest.param(
            "MergeTree()",
            "TTL d1 TO VOLUME 'external'",
            "MODIFY TTL d1 + INTERVAL 15 MINUTE TO DISK 'external'",
...
def test_layered_tired_storage(started_cluster, engine, request):
    ...
    try:
        ...
        node1.query(f"""
            ...
            TTL d1 + INTERVAL 0 SECOND TO DISK 'jbod2',
                d1 + INTERVAL {SOON_SECONDS} SECOND TO DISK 'external',
                d1 + INTERVAL {SOON_SECONDS*2} SECOND DELETE

        """)
        time.sleep(SOON_SECONDS+1)
        wait_until_move_is_finished(node1, table_name)
        stats = get_details_of_active_parts(node1, table_name)
        # NOTE: RMT could contain empty parts after removal - we have to handle that more carefully
        assert stats['jbod1'] == {'rows': 20}
        if 'external' in stats:
            assert stats['external'] == {'rows': 0}
...
def test_materialize_ttl_per_partition(started_cluster, engine, request):
    ...
    try:
        ...
        assert get_disk_names_of_active_parts(node1, table_name, partition=2) == {"external"}
        assert get_disk_names_of_active_parts(node1, table_name, partition=3) == {"jbod1"}
        assert get_disk_names_of_active_parts(node1, table_name, partition=4) == {"external"}
...
def test_replicated_download_ttl_info(started_cluster, request):
    ...
    try:
        ...
        assert get_details_of_active_parts(node2, table_name) == {
            'jbod1': {'rows': 10},
            'external': {'rows': 10},
        }
        ...
        wait_until_move_is_finished(node2, table_name)
        assert get_details_of_active_parts(node2, table_name) == {
            'external': {'rows': 20},
        }
        assert node2.query(f"SELECT count() FROM {table_name}").strip() == "20"
...
def test_moves_with_full_disk(started_cluster, engine, request):
    ...
    try:
        ...
        assert get_disk_names_of_active_parts(node1, temp_table_name) == {'jbod2'}
        ...
        assert get_disk_names_of_active_parts(node1, table_name) == {'jbod1'}
        ...
        time.sleep(1)
        wait_until_move_is_finished(node1, table_name)
        assert get_disk_names_of_active_parts(node1, table_name) == {'jbod2'}
